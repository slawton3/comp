...
export default function CocktailCard({ cocktail, ingredients, instructions }: ICocktail) {
  ...
  const createDetailedCard = (ingredients: Array<IIngredients>, instructions: string[]) => {
    return (
      <div>
        <StyledCard>
          <CardMedia
  ...
  return (
    <div>
      {ingredients && instructions ? createDetailedCard(ingredients, instructions) : createStandardCard()}
    </div>
  );
}